...
import com.spboot.app.mapper.ShoucangMapper;
...
public class ShoucangService {
     ...
     *  根据id 获取一行数据
     */
    public R<Shoucang> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Shoucang>> selectAll(Wrapper<Shoucang> query) {
     ...
     *  直接筛选所有数据
     */
    public R<List<Shoucang>> selectAll() {
    ...
    public R selectPages(QueryWrapper<Shoucang> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Shoucang entityData, Map post) {
        Info.handlerNullEntity(entityData);
        entityData.setUsername(SessionFactory.getUsername());
        entityData.setAddtime(Info.getDateStr());

        Map res = DB.name("shoucang").where("biao", post.get("biao")).where("xwid", post.get("xwid")).where("username", SessionFactory.getUsername()).find();
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Shoucang entityData, Map post) {
