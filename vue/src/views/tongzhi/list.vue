                ...
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        <el-form-item label="用户">
                            <el-input v-model="search.yonghu"></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column width="80" label="审核">
                        <template #default="{ row }">
                            {{ row.issh }}
                            <el-button size="small" :type="row.issh == '是' ? 'success' : 'danger'" @click="canTongzhiCheckIssh(row)">
                                {{ row.issh == "是" ? "取消审核" : "审核" }}
                            </el-button>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/tongzhi/detail?id=' + row.id)">详情</el-button> </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/tongzhi/updt?id=' + row.id)">编辑</el-button> </el-tooltip>
...
<script setup>
    ...
    import { canTongzhiSelect, useTongzhiSelect, canTongzhiDelete, canTongzhiCheckIssh } from "@/module";
