                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row @selection-change="handleSelectionChange">
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-button size="small" @click="$router.push('/' + row.biao + '/detail?id=' + row.xwid)">详情</el-button><el-tooltip effect="dark" content="删除" placement="top-start"><el-button type="danger" :icon="Delete" size="small" @click="deleteItems(row.id)">删除</el-button> </el-tooltip>
                            </el-button-group>
                        </template>
                    </el-table-column>
                </el-table>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/shoucang/selectUsername", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
