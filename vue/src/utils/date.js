...
export function getYesterday(isTime) {
    var time = getTime() - 86400;
    var format = "Y-m-d";
    if (isTime) {
        return [date("Y-m-d 00:00:00", time), date("Y-m-d 23:59:59", time)];
    } else {
        return [date("Y-m-d", time), date("Y-m-d", time)];
    }
}
...
function secondsToTime(seconds) {
    ...
    // 如果有天数，显示天数 + HH:mm:ss
    if (days > 0) {
        return `${days}天 ${format(hours)}:${format(minutes)}:${format(secs)}`;
    }
