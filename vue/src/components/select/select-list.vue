...
<script>
    import { empty, isArray, isObject, isFunction, isAsyncFunction, isPromise } from "@/utils/extend";
        ...
        methods: {
            ...
            async searchTable(isEmit) {
                ...
                if (isFunction(this.filters)) {
                    if (isAsyncFunction(this.filters)) {
                        await this.filters(query);
                    } else {
                        var res = this.filters(query);
                        if (isPromise(res)) {
