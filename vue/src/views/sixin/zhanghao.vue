<template>
    <div class="views-sixin-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 私信查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="sixinneirong" label="私信内容">
                        <template #default="{ row }"> {{ row.sixinneirong }} </template>
                    </el-table-column>
                    <el-table-column prop="tupian" label="图片" width="100">
                        <template #default="{ row }"> <e-img :src="row.tupian" style="max-width: 120px" /> </template>
                    </el-table-column>
                    <el-table-column prop="sixinren" label="私信人" width="180">
                        <template #default="{ row }"> {{ row.sixinren }} </template>
                    </el-table-column>
                    <el-table-column prop="sixinzhuangtai" label="私信状态" width="100">
                        <template #default="{ row }"> {{ row.sixinzhuangtai }} </template>
                    </el-table-column>
                    <el-table-column prop="addtime" label="发送时间">
                        <template #default="{ row }"> {{ row.addtime.substring(0, 19) }} </template>
                    </el-table-column>
                    <el-table-column prop="sixinhuifu" label="私信回复">
                        <template #default="{ row }"> {{ row.sixinhuifu }} </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/sixin/detail?id=' + row.id)">详情</el-button> </el-tooltip>
                                <template v-if="row.sixinzhuangtai == '未读'">
                                    <el-tooltip effect="dark" content="编辑" placement="top-start"><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/sixin/updt?id=' + row.id)">回复</el-button> </el-tooltip>
                                </template>
                            </el-button-group>
                        </template>
                    </el-table-column>
...
<script setup>
    ...
    import { canSixinSelect, useSixinSelect, canSixinDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/sixin/selectZhanghao", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
