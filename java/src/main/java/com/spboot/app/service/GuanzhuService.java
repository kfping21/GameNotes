...
public class GuanzhuService {
    ...
    public R<List<Guanzhu>> selectAll() {
        QueryWrapper<Guanzhu> wrapper = Wrappers.query();
    ...
    /**
     *  根据map 条件筛选guanzhuren字段等于session.username的数据并分页
     *
     */
    public R selectPagesGuanzhuren(Map<String, Object> map) {
        ...
        QueryWrapper<Guanzhu> wrapper = mapToWrapper(map);
        // 设置为当前角色
        wrapper.eq("guanzhuren", SessionFactory.getUsername());
        wrapper.orderBy(true, selectPage.isAsc(), selectPage.getOrderby());
        Page page = new Page(selectPage.getPage(), selectPage.getPagesize());
        return selectPages(wrapper, page);
    }
    ...
    public QueryWrapper<Guanzhu> mapToWrapper(Map<String, Object> map) {
        ...
        if (!StringUtil.isNullOrEmpty(map.get("guanzhuren"))) {
            wrapper.like("guanzhuren", map.get("guanzhuren"));
        }
     ...
     * @return 是否处理成功
     */
    public R insert(Guanzhu entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("UPDATE biji SET guanzhuliang = guanzhuliang+1 WHERE id='" + post.get("bijiid") + "'");

            DB.execute("INSERT INTO tongzhi(tongzhineirong, yonghu, issh) VALUES ('恭喜你笔记有新的关注-请查看： ', '" + post.get("tianjiaren") + "', '否')");
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Guanzhu entityData, Map post) {
    ...
    public R<Object> delete(Integer id) {
        try {
            Guanzhu map = mapper.selectById(id);
            Map post = DB.name("guanzhu").find(id);
