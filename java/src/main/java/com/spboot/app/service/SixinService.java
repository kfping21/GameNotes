...
public class SixinService {
    ...
    // 获取数据库操作类mapper
    @Resource
    private SixinMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Sixin> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Sixin>> selectAll(Wrapper<Sixin> query) {
    ...
    public R<List<Sixin>> selectAll() {
        QueryWrapper<Sixin> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Sixin> wrapper = mapToWrapper(map);
        // 设置排序
    ...
    /**
     *  根据map 条件筛选sixinren字段等于session.username的数据并分页
     *
     */
    public R selectPagesSixinren(Map<String, Object> map) {
        // 获取前端给到列表基础参数
        SelectPage selectPage = new SelectPage(map, 10, "id", "DESC");
        // 将map参数转换为mybatis-plus的QueryWrapper类
        QueryWrapper<Sixin> wrapper = mapToWrapper(map);
        // 设置为当前角色
        wrapper.eq("sixinren", SessionFactory.getUsername());
        wrapper.orderBy(true, selectPage.isAsc(), selectPage.getOrderby());
        Page page = new Page(selectPage.getPage(), selectPage.getPagesize());
        return selectPages(wrapper, page);
    }
     ...
     */
    public QueryWrapper<Sixin> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Sixin> wrapper = Wrappers.query();
    ...
    public R selectPages(QueryWrapper<Sixin> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Sixin entityData, Map post) {
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Sixin entityData, Map post) {
        mapper.updateById(entityData);

        Integer charuid = entityData.getId();
        DB.execute("UPDATE sixin SET sixinzhuangtai = '已回复' WHERE id='" + charuid + "'");
