...
public class XiaoxiService {
    ...
    // 获取数据库操作类mapper
    @Resource
    private XiaoxiMapper mapper;

    /**
     *  根据id 获取一行数据
     */
    public R<Xiaoxi> findById(Integer id) {
     ...
     *  根据Wrapper 对象进行数据筛选
     */
    public R<List<Xiaoxi>> selectAll(Wrapper<Xiaoxi> query) {
    ...
    public R<List<Xiaoxi>> selectAll() {
        QueryWrapper<Xiaoxi> wrapper = Wrappers.query();
    ...
    public R selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Xiaoxi> wrapper = mapToWrapper(map);
        // 设置排序
    ...
    /**
     *  根据map 条件筛选fasongren字段等于session.username的数据并分页
     *
     */
    public R selectPagesFasongren(Map<String, Object> map) {
        // 获取前端给到列表基础参数
        SelectPage selectPage = new SelectPage(map, 10, "id", "DESC");
        // 将map参数转换为mybatis-plus的QueryWrapper类
        QueryWrapper<Xiaoxi> wrapper = mapToWrapper(map);
        // 设置为当前角色
        wrapper.eq("fasongren", SessionFactory.getUsername());
        wrapper.orderBy(true, selectPage.isAsc(), selectPage.getOrderby());
        Page page = new Page(selectPage.getPage(), selectPage.getPagesize());
        return selectPages(wrapper, page);
    }
     ...
     */
    public QueryWrapper<Xiaoxi> mapToWrapper(Map<String, Object> map) {
        // 创建 QueryWrapper 对象
        QueryWrapper<Xiaoxi> wrapper = Wrappers.query();
        ...
        // 判断URL 参数siliaoid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("siliaoid")) && Convert.toInt(map.get("siliaoid")) > 0) {
            // 大于0 则写入条件
            wrapper.eq("siliaoid", map.get("siliaoid"));
        }
    ...
    public R selectPages(QueryWrapper<Xiaoxi> wrapper, IPage page) {
     ...
     * @return 是否处理成功
     */
    public R insert(Xiaoxi entityData, Map post) {
        // 判断是否有填写发送时间。
        if (StringUtil.isNullOrEmpty(post.get("fasongshijian"))) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("UPDATE siliao SET addtime = now() WHERE id='" + post.get("siliaoid") + "'");
     ...
     * @return 是否处理成功
     */
    public R<Object> update(Xiaoxi entityData, Map post) {
        // 判断是否有填写发送时间。
        if (StringUtil.isNullOrEmpty(post.get("fasongshijian"))) {
            return R.error("请填写发送时间");
        }
