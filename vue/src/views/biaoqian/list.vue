<template>
    <div class="views-biaoqian-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 标签查询 </span>
                    </div>
                </template>
                ...
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        <el-form-item label="标签名称">
                            <el-input v-model="search.biaoqianmingcheng"></el-input>
                        </el-form-item>
                        <el-form-item>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="biaoqianmingcheng" label="标签名称">
                        <template #default="{ row }"> {{ row.biaoqianmingcheng }} </template>
                    </el-table-column>

                    <el-table-column label="操作" fixed="right" width="250">
                        <template #default="{ row }">
                            <el-button-group>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/biaoqian/updt?id=' + row.id)">编辑</el-button> </el-tooltip>
...
<script setup>
    ...
    import { canBiaoqianSelect, useBiaoqianSelect, canBiaoqianDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canBiaoqianDelete(ids).then((res) => {
                        if (res.code == 0) {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/biaoqian/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
